...
import { doctorSrdFramework } from "./lib/doctor.js";
import { installSrdFramework, InstallConflictError } from "./lib/install.js";
import { resolvePackageRoot, readPackageMetadata } from "./lib/paths.js";
import { uninstallSrdFramework } from "./lib/uninstall.js";
import { updateSrdFramework } from "./lib/update.js";
...
function parseArgs(argv: string[]): ParsedArgs {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (value === "--config-dir") {
      parsed.configDir = argv[index + 1];
      index += 1;
      continue;
    }

    if (value.startsWith("--config-dir=")) {
      parsed.configDir = value.slice("--config-dir=".length);
      continue;
    }
  ...
  if (positional[0]) {
    parsed.command = positional[0] as CommandName;
  }
...
async function main(): Promise<void> {
  ...
  const binaryName = basename(process.argv[1] || "opencode-srd-framework");
  ...
  switch (args.command) {
    case "install": {
      const result = await installSrdFramework(sharedOptions);
      printHumanResult(result);
      return;
    }
    case "update": {
      const result = await updateSrdFramework(sharedOptions);
      printHumanResult(result);
      return;
    }
    case "uninstall": {
      const result = await uninstallSrdFramework(sharedOptions);
      printHumanResult(result);
      return;
    }
    case "doctor": {
      const result = await doctorSrdFramework({
        configDir: args.configDir,
        packageRoot,
      });
