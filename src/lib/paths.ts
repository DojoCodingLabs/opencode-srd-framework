...
export const PACKAGE_NAME = "@dojocoding/opencode-srd-framework";
export const CLI_NAME = "opencode-srd-framework";
export const CONFIG_SCHEMA_URL = "https://opencode.ai/config.json";
export const MANIFEST_FILE_NAME = ".dojocoding-opencode-srd-framework.manifest.json";
export const DEFAULT_REMINDER_INTERVAL_MS = 3 * 60 * 1000;
...
export function resolvePackageRoot(moduleUrl: string): string {
  ...
  while (true) {
    ...
    const assetsPath = join(currentDirectory, "assets");

    if (existsSync(packageJsonPath) && existsSync(assetsPath)) {
