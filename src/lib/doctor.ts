...
export async function doctorSrdFramework(options: DoctorOptions): Promise<DoctorResult> {
  ...
  const managedFiles = new Set(manifest?.managedFiles.map((file) => file.relativePath) ?? []);
